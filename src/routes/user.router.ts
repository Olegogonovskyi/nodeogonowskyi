...
import {ItemValidator} from "../validators/customerValidator";

const router = Router()

router.get('/', authMiddleware.checkAccesToken, commonMiddleware.isQueryValid(ItemValidator.getListUsers), userControllers.getAll)
router.get('/customers', authMiddleware.checkAccesToken, commonMiddleware.isQueryValid(ItemValidator.getListCustomers), userControllers.getAllCustomers)
